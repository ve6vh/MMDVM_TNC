...
// capture the FSK DEMOD output
#define	CAPTURE_DEMOD_OUTPUT		0				// set to one to capture DAC output
#define CAPTURE_DEMOD_SAMPLE		0				// capture the input sample
#define	CAPTURE_ADC_SAMPLES			0

#if CAPTURE_DEMOD_OUTPUT || CAPTURE_DEMOD_SAMPLE || CAPTURE_ADC_SAMPLES
#define	DEMOD_CAPTURE_SIZE		8000
uint32_t  demodBuf[DEMOD_CAPTURE_SIZE];
uint32_t  *pdemodBuf = demodBuf;
#endif
...
void CMode2RX::samples(q15_t* samples, uint8_t length)
{
  ...
  for (uint8_t i = 0U; i < length; i++) {
    ...
    // for debug purposes only; capture the demod buffer when squelch is open
#if  CAPTURE_DEMOD_OUTPUT || CAPTURE_DEMOD_SAMPLE || CAPTURE_ADC_SAMPLES
    if(CIO.getSquelch())		{
#if CAPTURE_DEMOD_OUTPUT
    	for(int k=0;k<MODE2_RADIO_SYMBOL_LENGTH;k++)
    		*pdemodBuf++ = (uint32_t)m_bitBuffer[k];
#endif
#if CAPTURE_DEMOD_SAMPLE
		*pdemodBuf++ = (uint32_t)sample;
#endif
#if CAPTURE_ADC_SAMPLES
		*pdemodBuf++ = (uint32_t)samples[i];
#endif
    	if((pdemodBuf-demodBuf) > DEMOD_CAPTURE_SIZE)
    		pdemodBuf = demodBuf;
    }
#endif
